...
const attachmentResponse = await skribble.attachment.add(signature_request_id, attachments);
```
...
<ParamField path="attachments" type="array" required>
  List of objects containing attachment information
...
```typescript
const attachmentContent = await skribble.attachment.get(signature_request_id, attachment_id);
```
...
```typescript
await skribble.attachment.delete(signature_request_id, attachment_id);
```
