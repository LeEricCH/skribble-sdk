...
```typescript
await skribble.signature_request.create(signature_request);
```
...
```typescript
await skribble.signature_request.get(signature_request_id);
```
...
```typescript
const signatureRequests = await skribble.signature_request.list({
...
<ParamField path="page_number" type="number">
...
```typescript
await skribble.signature_request.delete(signature_request_id);
```
...
```typescript
await skribble.signature_request.withdraw(signature_request_id, { message: "Withdrawing for testing purposes" });
```
...
```typescript
const addSignerResponse = await skribble.signature_request.add_signer(
...
```typescript
const signatureRequest = await skribble.signature_request.get(signature_request_id);

const signerToRemove = signatureRequest.signatures[0]; // This is the first signer in the list

await skribble.signature_request.remove_signer(signature_request_id, signerToRemove.sid);
```
...
await skribble.signature_request.replace_signers(signature_request_id, newSigners);
```
...
<ParamField path="signatures" type="array" required>
  List of objects containing new signer information
...
To add an attachment to a signature request:

```typescript
const attachment: Types.AttachmentRequest = {
    filename: "doc1.pdf",
    content_type: "application/pdf",
    content: "base64_encoded_content"
};
const attachmentResponse = await skribble.signature_request.attachment.add(signature_request_id, attachment);
```

<ParamField path="signature_request_id" type="string" required>
  ID of the signature request to add the attachment to
</ParamField>

<ParamField path="attachment" type="AttachmentRequest" required>
  <Expandable title="properties">
...
```typescript
const attachmentContent = await skribble.signature_request.attachment.get(signature_request_id, attachment_id);
```
...
```typescript
await skribble.signature_request.attachment.delete(signature_request_id, attachment_id);
```
...
```typescript
const attachments = await skribble.signature_request.attachment.list(signature_request_id);
```
