...
USERNAME: str = os.getenv('SKRIBBLE_USERNAME')
API_KEY: str = os.getenv('SKRIBBLE_API_KEY')

if not USERNAME or not API_KEY:
    raise ValueError(
        "Please set your Skribble API credentials in the .env file.\n"
...
try:
    ...
    document_content = skribble.document.download(document_id, content_type="blob")
    print(colored("Downloaded document (blob) size:", "green"), colored(len(document_content), "cyan"), colored("bytes", "green"))

    # Download document as base64
    document_content_base64 = skribble.document.download(document_id, content_type="base64")
    print(colored("Downloaded document (base64) size:", "green"), colored(len(document_content_base64), "cyan"), colored("characters", "green"))
